...
In a [previous series of posts](/pbt), I introduced you to the burned-out, lazy and malicious programmer known as the *Enterprise Developer From Hell*, or the *EDFH* for short.
...
Since this is the specification we are given, this is a perfectly fine answer!

It's funny though, because the interviewer was obviously asking for a more complex implementation.
...
So now we have these two challenges:

* What *should* the specification for RLE be? How can we define it in an unambiguous way?
* And then, how can we check that a particular RLE implementation meets that spec?
...
A nice thing about property-based testing is that, by doing it, you can often discover the specification. In my previous post, I discussed how we might test an implementation of addition. Eventually, we discovered the properties of commutativity, associativity, and identity, which pretty much define what "addition" actually is.
...
But this is the hard part -- thinking of properties. However, we can use the EDFH to help us! What would the EDFH do, and why would it be wrong?
...
Ok then, is there something else we can add? What about those counts? They look wrong. What should they be?
...
* The sum of the run lengths in the output must equal the length of the input
* If the input is reversed, the output must also be reversed

Is that enough to correctly check a RLE implementation? Can you think of any malicious EDFH implementations that would satisfy these properties and yet be wrong?
...
...we immediately get a `ArgumentNullException` because we completely forgot to handle null inputs in the implementation! Thank you, property based test!
...
So the `rle_allChars` implementation does pass, as we expected. Which is why we need the next property: "No two adjacent characters in the output can be the same"
...
And... it unexpectedly passes.

```
Ok, passed 100 tests.
```

Oh no. Maybe we were just unlucky? Let's change the default configuration to be 10000 runs rather than 100.
...
And...it still passes. That's not good.
...
Here's what the input strings look like:
...
We can see that the strings are very random, but almost never have a series of repeated characters. From the point of view of testing an RLE algorithm these inputs are completely useless!
...
That will be the topic of the [next installment here](/posts/return-of-the-edfh-2)

*Source code for this post is [available here](https://github.com/swlaschin/fsharpforfunandprofit.com_code/posts/return-of-the-edfh)*
